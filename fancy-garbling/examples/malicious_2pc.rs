...
fn run_circuit(circ: &mut Circuit, gb_inputs: Vec<u16>, ev_inputs: Vec<u16>) {
    ...
    let m = gb_inputs.len();
    let bs = ev_inputs.iter().map(|v| if *v == 0 as u16 { false } else { true } ).collect::<Vec<bool>>();
    ...
    let handle = std::thread::spawn(move || {
        ...
        let mut gb = Garbler::<UnixChannel, AesRng>::new(sender, m, crs_, rng).unwrap();
        ...
        println!("Garbler :: Encoding inputs: {} ms", start.elapsed().unwrap().as_millis());
        let start = SystemTime::now();
        circ_.eval(&mut gb, &xs, &ys).unwrap();
        println!("Garbler :: Circuit garbling: {} ms", start.elapsed().unwrap().as_millis());
    });
    let rng = AesRng::new();
    let start = SystemTime::now();
    let mut ev = Evaluator::<UnixChannel, AesRng>::new(receiver, &bs, &crs, rng).unwrap();
    ...
    println!("Evaluator :: Encoding inputs: {} ms", start.elapsed().unwrap().as_millis());
    let start = SystemTime::now();
    circ.eval(&mut ev, &xs, &ys).unwrap();
    println!("Evaluator :: Circuit evaluation: {} ms", start.elapsed().unwrap().as_millis());
    handle.join().unwrap();
