...
pub fn create_random_proof<E: PairingEngine, R: Rng>(
    ...
    // Number of io variables (public statements)
    ...
    println!("total inputs: {:?}, total aux: {:?}, total variables: {:?}", m_io * n, m_mid * n, m * n);
    println!("total constraints: {:?}", 2 * n);
    ...
    let size = mem::size_of_val(&(proof.r_mid_comms))
            + mem::size_of_val(&(proof.q_comm))
            + mem::size_of_val(&(proof.r_mid_q_values))
            + mem::size_of_val(&(proof.opening_challenge));
