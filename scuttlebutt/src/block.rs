...
impl Block {
    ...
    // #[cfg(feature = "curve25519-dalek")]
    #[inline]
    pub fn hash_pt(tweak: u128, pt: &G1Affine) -> Self {
        // let k = pt.compress();
        // let c = Aes256::new(k.as_bytes());
        // c.encrypt(Block::from(tweak))
        // TODO: length
        // let xl_block = Block::try_from_slice(&k.as_bytes()[..16]);
        let mut xl_vec = vec![];
        pt.write(&mut xl_vec).unwrap();
        let mut xl_array = [0u8; 16]; 
        xl_array.copy_from_slice(&xl_vec[..16]);
        let xl_block = Block::from(xl_array);
        let xr_block = Block::from(tweak);
        hash_mimc::mimc5_hash_block(xl_block, xr_block)
    }
